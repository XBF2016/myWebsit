...
public class MessageDaoImpl extends HibernateDaoSupport implements MessageDao {
	...
	public Message selectBean(String where) {
		List<Message> list = this.getHibernateTemplate().find(
				"from Message " + where);
		if (list.size() == 0) {
			return null;
		}
	...
	public int selectBeanCount(String where) {
		long count = (Long) this.getHibernateTemplate()
				.find("select count(*) from Message " + where).get(0);
		return (int) count;
	}

	@SuppressWarnings("unchecked")
	public List<Message> selectBeanList(final int start, final int limit,
			final String where) {
		return (List<Message>) this.getHibernateTemplate().executeFind(
				new HibernateCallback() {
					public Object doInHibernate(final Session session)
							throws HibernateException, SQLException {
						List<Message> list = session
								.createQuery("from Message " + where)
