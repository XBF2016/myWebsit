...
public class CompanyDaoImpl extends HibernateDaoSupport implements CompanyDao {
	...
	public Company selectBean(String where) {
		List<Company> list = this.getHibernateTemplate().find(
				"from Company " + where);
		if (list.size() == 0) {
			return null;
		}
	...
	public int selectBeanCount(String where) {
		long count = (Long) this.getHibernateTemplate()
				.find("select count(*) from Company " + where).get(0);
		return (int) count;
	}

	@SuppressWarnings("unchecked")
	public List<Company> selectBeanList(final int start, final int limit,
			final String where) {
		return (List<Company>) this.getHibernateTemplate().executeFind(
				new HibernateCallback() {
					public Object doInHibernate(final Session session)
							throws HibernateException, SQLException {
						List<Company> list = session
								.createQuery("from Company " + where)
